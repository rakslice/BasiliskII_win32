...
#ifdef WIN32
// #define HAS_FILE_SHARING
#include "posix_emu.h"
#endif

#ifdef HAS_FILE_SHARING
...
#define ALBLKSIZE 16384
#define CLUMPSIZE	16384
...
static void dump_FSItem_list( char *descr )
{
	D(bug("CNID tree %s\n",descr));
	FSItem *p = first_fs_item;
	while(p) {
		D(bug("p=%08x, name=%s, id=%ld, parent p=%08x, parent id=%ld\n",p,p->name,p->id,p->parent,p->parent_id));
		p = p->next;
	}
}

static void replace_parent_ids( uint32 old_parent, uint32 new_parent )
{
	FSItem *p = first_fs_item;
	while(p) {
		if(p->parent_id == old_parent) p->parent_id = new_parent;
		else if(p->parent_id == new_parent) p->parent_id = old_parent;
		p = p->next;
	}
}
...
static int16 fs_volume_mount(uint32 pb)
{
	...
	WriteMacInt32(vcb + vcbAlBlkSiz, ALBLKSIZE);
...
//	D(bug(" fs_get_vol_info(%08lx)\n", pb));
	...
	WriteMacInt32(pb + ioVAlBlkSiz, ALBLKSIZE);
	WriteMacInt32(pb + ioVClpSiz, CLUMPSIZE);
...
{
	// D(bug(" fs_get_vol_parms(%08lx), ioReqCount=%08lx\n", pb, ReadMacInt32(pb + ioReqCount)));

	// Return parameter block

	uint8 vol[SIZEOF_GetVolParmsInfoBuffer];
	*((uint16 *)&vol[vMVersion]) = ntohs(2);
	*((uint32 *)&vol[vMAttrib]) = ntohl(kNoMiniFndr | kNoVNEdit | kNoLclSync | kTrshOffLine | kNoSwitchTo | kNoBootBlks | kNoSysDir | kHasExtFSVol);
	*((uint32 *)&vol[vMLocalHand]) = ntohl(0);
	*((uint32 *)&vol[vMServerAdr]) = ntohl(0);
	*((uint32 *)&vol[vMVolumeGrade]) = ntohl(0);
	*((uint16 *)&vol[vMForeignPrivID]) = ntohs(0);
...
read_next_de:
	...
	uint32 location;
	get_finder_location(full_path, location);
	WriteMacInt32(pb + ioFlFndrInfo + fdLocation, location);

	uint16 fldr;
	get_finder_fldr(full_path, fldr);
	WriteMacInt16(pb + ioFlFndrInfo + fdFldr, fldr);

	WriteMacInt16(pb + ioFlStBlk, 0);
	WriteMacInt32(pb + ioFlLgLen, st.st_size);
	WriteMacInt32(pb + ioFlPyLen, (st.st_size + ALBLKSIZE-1) & ~(ALBLKSIZE-1));
	WriteMacInt16(pb + ioFlRStBlk, 0);
	uint32 rf_size = get_rfork_size(full_path);
	WriteMacInt32(pb + ioFlRLgLen, rf_size);
	WriteMacInt32(pb + ioFlRPyLen, (rf_size + ALBLKSIZE-1) & ~(ALBLKSIZE-1));
...
static int16 fs_set_file_info(uint32 pb, bool hfs, uint32 dirID)
{
	...
	set_finder_location(full_path, ReadMacInt32(pb + ioFlFndrInfo + fdLocation));
	set_finder_fldr(full_path, ReadMacInt16(pb + ioFlFndrInfo + fdFldr));
...
read_next_de:
...
#ifdef WIN32
	// temp only; something goes wrong with caching
	cached = false;
#endif
	...
	if (S_ISDIR(st.st_mode)) {
		...
		uint32 rect_lo, rect_hi;
		get_finder_rect(full_path, rect_lo, rect_hi);
		WriteMacInt32(pb + ioDrUsrWds + frRect, rect_lo);
		WriteMacInt32(pb + ioDrUsrWds + frRect + 4, rect_hi);

		uint32 location;
		get_finder_location(full_path, location);
		WriteMacInt32(pb + ioDrUsrWds + frLocation, location);

		uint16 view;
		get_finder_view(full_path, view);
		WriteMacInt16(pb + ioDrUsrWds + frView, view);
...
static int16 fs_set_cat_info(uint32 pb)
{
	...
	if (S_ISDIR(st.st_mode)) {
		...
		set_finder_rect(full_path, ReadMacInt32(pb + ioDrUsrWds + frRect), ReadMacInt32(pb + ioDrUsrWds + frRect + 4));
		set_finder_location(full_path, ReadMacInt32(pb + ioDrUsrWds + frLocation));
		set_finder_view(full_path, ReadMacInt16(pb + ioDrUsrWds + frView));
	} else {
		set_finder_type(full_path, ReadMacInt32(pb + ioFlFndrInfo + fdType), ReadMacInt32(pb + ioFlFndrInfo + fdCreator));
		set_finder_flags(full_path, ReadMacInt16(pb + ioFlFndrInfo + fdFlags));
		set_finder_location(full_path, ReadMacInt32(pb + ioFlFndrInfo + fdLocation));
...
static int16 fs_open(uint32 pb, uint32 dirID, uint32 vcb, bool resource_fork)
{
...
#ifdef HAS_FILE_SHARING
			// Ignoring the sharing modes causes some apps not working as they should.
			do_lock = 1;
			// fall
#endif
...
#ifdef HAS_FILE_SHARING
	if(fd >= 0 && do_lock) {
		if (locking(fd, LK_NBLCK,0x7FFFFFFF) < 0) {
			close(fd);
			return errno2oserr();
		}
		// locking should be released before closing a file.
	}
#endif
	...
	WriteMacInt32(fcb + fcbPLen, (st.st_size + ALBLKSIZE-1) & ~(ALBLKSIZE-1));
	WriteMacInt32(fcb + fcbCrPs, 0);
	WriteMacInt32(fcb + fcbVPtr, vcb);
	WriteMacInt32(fcb + fcbClmpSize, CLUMPSIZE);
...
static int16 fs_get_eof(uint32 pb)
{
	...
	D(bug(" fs_get_eof::fcbFlNm = %08lx\n", ReadMacInt32(fcb + fcbFlNm)));
	...
	D(bug(" fs_get_eof::fd = %d\n", fd));

	if (fd < 0) {

		D(bug(" fs_get_eof::fcbFlags = %08lx\n", ReadMacInt32(fcb + fcbFlags)));
...
static int16 fs_set_eof(uint32 pb)
{
	...
	WriteMacInt32(fcb + fcbPLen, (size + ALBLKSIZE-1) & ~(ALBLKSIZE-1));
	D(bug("  adjusting FCBs\n"));
	r.d[0] = ReadMacInt16(pb + ioRefNum);
	Execute68k(fs_data + fsAdjustEOF, &r);
	D(bug("  UTAdjustEOF() returned %d\n", r.d[0]));
...
static int16 fs_read(uint32 pb)
{
	...
	if( (int32)ReadMacInt32(pb + ioReqCount) < 0) return paramErr;
...
{
	D(bug(" fs_write(%08lx), refNum %d, buffer %p, count %d, posMode %d, posOffset %d\n", pb, ReadMacInt16(pb + ioRefNum), ReadMacInt32(pb + ioBuffer), ReadMacInt32(pb + ioReqCount), ReadMacInt16(pb + ioPosMode) & 3, ReadMacInt32(pb + ioPosOffset)));
...
static int16 fs_create(uint32 pb, uint32 dirID)
{
...
#if DEBUG
	dump_FSItem_list("Before fs_create");
#endif
	...
	if (fd < 0)
		...
		close(fd);
#if DEBUG
		dump_FSItem_list("After fs_create");
#endif
		return noErr;
	}
}
...
static int16 fs_dir_create(uint32 pb)
{
...
#if DEBUG
	dump_FSItem_list("Before fs_dir_create");
#endif
	...
	if (mkdir(full_path, 0777) < 0)
		...
		WriteMacInt32(pb + ioDirID, fs_item->id);
#if DEBUG
		dump_FSItem_list("After fs_dir_create");
#endif
		return noErr;
	}
}
...
static int16 fs_delete(uint32 pb, uint32 dirID)
{
	...
	if (remove(full_path) < 0) {
		...
		get_finf_path(full_path, helper_path, false);
		remove(helper_path); // may silently fail
		get_rsrc_path(full_path, helper_path, false);
		remove(helper_path); // may silently fail
		return noErr;
	}
}
...
static int16 fs_rename(uint32 pb, uint32 dirID)
{
...
#if DEBUG
	dump_FSItem_list("Before fs_rename");
#endif
	...
	uint8 save_pb[SIZEOF_IOParam];
	memcpy(save_pb, Mac2HostAddr(pb), SIZEOF_IOParam);

	// reuse pb area, to be restored a few lines down
	WriteMacInt32((uint32)pb + ioNamePtr, ReadMacInt32(pb + ioMisc));
	FSItem *new_item;
	result = get_item_and_path((uint32)pb, dirID, new_item);
	memcpy(Mac2HostAddr(pb), save_pb, SIZEOF_IOParam);
	...
	if (rename(old_path, full_path) < 0)
		...
		char helper_path_src[MAX_PATH_LENGTH], helper_path_dst[MAX_PATH_LENGTH];
		get_finf_path(old_path, helper_path_src, false);
		if (access(helper_path_src, F_OK) == 0) {
			get_finf_path(full_path, helper_path_dst, true);
			rename(helper_path_src, helper_path_dst);
		}
		get_rsrc_path(old_path, helper_path_src, false);
		if (access(helper_path_src, F_OK) == 0) {
			get_rsrc_path(full_path, helper_path_dst, true);
			rename(helper_path_src, helper_path_dst);
		}

		// The ID of the old file/dir has to stay the same, so we swap the IDs of the FSItems
		replace_parent_ids( fs_item->id, new_item->id );
		uint32 t = fs_item->id;
		fs_item->id = new_item->id;
		new_item->id = t;

#if DEBUG
		dump_FSItem_list("After fs_rename");
#endif
...
static int16 fs_cat_move(uint32 pb)
{
...
#if DEBUG
	dump_FSItem_list("Before fs_cat_move");
#endif
	...
	// Reuse the pb area, to be restored below
	WriteMacInt32((uint32)pb + ioNamePtr, ReadMacInt32(pb + ioNewName));
	FSItem *new_dir_item;
	result = get_item_and_path((uint32)pb, ReadMacInt32(pb + ioNewDirID), new_dir_item);
	memcpy(Mac2HostAddr(pb), save_pb, SIZEOF_IOParam);
...
#if DEBUG
		dump_FSItem_list("After fs_cat_move");
#endif
		return noErr;
	}
}
