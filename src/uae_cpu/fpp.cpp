  ...
  * FDBcc:
	...
	*
	*
	* This file is not used by the Windows port anymore.
	* .\Windows\fpu.cpp is used instead.
	*
  */
...
static bool is_integral_68040_fpu = false;
...
extern uint16 debugging_fpp;
uint16 debugging_fpp = 1;

void toggle_fpp_debug( void )
{
	debugging_fpp = !debugging_fpp;
}
...
// This does actually no good, Windows does not support 80 bit floats
...
static __inline__ int get_fp_value (uae_u32 opcode, uae_u16 extra, double *src)
{
  ...
  switch (size) {
	...
	D(bug("get_fp_value m68k_getpc() at end=%X\r\n",m68k_getpc()));
...
void REGPARAM2 fpp_opp(uae_u32 opcode, uae_u16 extra)
{
  ...
  D(bug("FPP %04lx %04x at %08lx\r\n", opcode & 0xffff, extra & 0xffff,
	...
	// Clear exception byte and accrued exception byte
	// regs.fpsr &= 0xFFFF0000;
				...
				if (incr < 0) {
					...
					for(reg=7; reg>=0; reg--) {
						...
						if( list & 0x80 ) {
							...
							ad -= 4;
							wrd1 = get_long (ad);
							// regs.fp[reg] = to_exten_no_normalize (wrd1, wrd2, wrd3);
...
{
	is_integral_68040_fpu = is_integral;
}
