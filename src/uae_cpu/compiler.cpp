...
// Keywords: TODO, MAYBEDONE, DOITLATER

#define DUMP_CODE 0
#define DISASSEMBLE_68K 0
#define COMPILE_ONLY 1
#define BREAK_AT_CODE_BLOCK 0
#define MAKE_TAGS 0
#define USER_PROGRAMS_BEHAVE 0

static int n_max_comp = 99999999;

// static int n_max_comp = 16,30; // ok
// static int n_max_comp = -1;

// static int n_max_comp = 80; // ok
...
#if MAKE_TAGS
#define TAG(x) \
	assemble( (uae_u8)0xEB );  \
	assemble( (uae_u8)4 );		 \
	assemble_ulong_68k(x)
#else
#define TAG(x)
#endif
...
extern void op_illg_1 (void) REGPARAM;
#define op_illg_1 ((cpuop_func *)op_illg_1)
#else
extern unsigned long REGPARAM2 op_illg_1 (uae_u32 opcode) REGPARAM;
#endif
...
static uae_s64 compiled_hits = 0;
...
#ifdef WIN32
		ncp = (struct code_page *)VirtualAlloc( 0, PAGE_ALLOC_UNIT, MEM_RESERVE|MEM_COMMIT, PAGE_EXECUTE_READWRITE );
#else
...
// TODO: fix get_hash() so that SCAN_MARKs other than 1 would work.

#ifdef WIN32
static int SCAN_MARK = 1;			/* Number of calls after which to scan a function */
static int COMPILE_MARK = 20; /* Number of calls after which to compile a function */
#else
static int SCAN_MARK = 1; /* Number of calls after which to scan a function */
...
static __inline__ int check_block(struct hash_block *hb)
{
    ...
    do {
        struct hash_entry *next = h->next_same_block;
//DOITLATER: this checksum is not good enough (Lauri)
...
uae_u32 flush_icache(void)
{
    ...
    while (hb != &lru_first_block) {
        ...
        if (hb->cpage != NULL) {
//DOITLATER
// Check this out, this condition seems to be wrong (Lauri)
...
//DOITLATER
// What's the purpose of this, nobody uses the return value (Lauri)
...
void REGPARAM2 m68k_setpc(uaecptr newpc)
{
    m68k_setpc_hash(newpc);
}

void REGPARAM2 m68k_setpc_fast(uaecptr newpc)
{
    m68k_setpc_nohash(newpc);
}

void REGPARAM2 m68k_setpc_rte(uaecptr newpc)
{
    m68k_setpc_nohash(newpc);
}

void REGPARAM2 m68k_setpc_bcc(uaecptr newpc)
{
    m68k_setpc_hash(newpc);
}
                                              ...
                                              wordsizes size)
{
    switch (m) {
     ...
     case Ad8r:
     case PC8r:
				// 68020 step over (Lauri)
        {  
            uae_u16 dp = *(*pcpp)++;
            dp <<= 8;
            dp |= *(*pcpp)++;
						if (dp & 0x100) {
							if ((dp & 0x30) == 0x20) (*pcpp)+=2;
							else if ((dp & 0x30) == 0x30) (*pcpp)+=4;
							if ((dp & 0x3) == 0x2) (*pcpp)+=2;
							else if ((dp & 0x3) == 0x3) (*pcpp)+=4;
						}
        }
        break;
     case absl:
...
static struct instr *translate_getnextinsn(uae_u8 **pcpp)
{
    ...
    opcode |= *(*pcpp)++;
		...
		// cft_map() was missing (Lauri)
		// op_illg was wrong, op_illg_1() is ok (Lauri)
#ifdef STREAMLINED_UAE
    if (real_cpufunctbl[cft_map(opcode)] == op_illg_1) {
#else
    ...
    dp = table68k + opcode;
    if (dp->suse) {
        translate_step_over_ea(pcpp, (amodes)dp->smode, (wordsizes)dp->size);
    }
    if (dp->duse) {
        translate_step_over_ea(pcpp, (amodes)dp->dmode, (wordsizes)dp->size);
    }
    return dp;
}
...
#ifdef WIN32
static int __cdecl bcc_compfn(const void *a, const void *b)
#else
...
#ifdef WIN32
static int __cdecl bb_compfn(const void *a, const void *b)
#else
...
static int find_basic_blocks(struct hash_entry *h)
{
    ...
    while (top_bb > current_bb) {
        ...
        for(;;) {
            ...
            if (dp->mnemo == i_DBcc) {
								// cast (Lauri)
                uaecptr newaddr = thisinsn_addr + 2 + (uae_s16)(((uae_u16)*(rpc_save+2) << 8) | *(rpc_save+3));
                if (!new_bcc_target(nextinsn_addr))
                    return 0;
            ...
            if (dp->mnemo == i_Bcc) {
                ...
                if (dp->smode == imm1)
										// cast (Lauri)
                    newaddr = thisinsn_addr + 2 + (uae_s16)(((uae_u16)*(rpc_save+2) << 8) | *(rpc_save+3));
                else if (dp->smode == imm2)
										// added 68020 imm2 modes (Lauri)
                    newaddr = thisinsn_addr + 2 + (uae_s32)(((uae_u32)*(rpc_save+2) << 24) | ((uae_u32)*(rpc_save+3) << 16) | ((uae_u32)*(rpc_save+4) << 8) | *(rpc_save+5));
                else
...
static __inline__ void assemble_ulong(uae_u32 a)
{
    assemble((uae_u8)a);
    assemble((uae_u8)(a >> 8));
    assemble((uae_u8)(a >> 16));
    assemble((uae_u8)(a >> 24));
}
...
// (Lauri)
static int lea_mult[] = { 0, 0x40, 0x80, 0xC0 };

/*
 * Load all the registers absolutely needed to calculate and verify the
                              ...
                              int pidmult)
{
    ...
    // int pdival = size == sz_byte && reg != 7 ? 1 : size == sz_long ? 4 : 2;
		int pdival = pidmult << (size == sz_byte && reg != 7 ? 0 : size == sz_long ? 2 : 1);
    ...
    switch(mode){
     ...
     case Ad8r:
				// Enhanced for 68020 modes. (Lauri)
        dp = (uae_s16)do_get_mem_word((uae_u16 *)p);
        (*pcpp) += 2; p += 2;
				r = (dp >> 12) & 7;
				int regd_shift, is_dreg;
				regd_shift = (dp >> 9) & 3;
				is_dreg = dp & 0x8000 ? 0 : 1;
			  ...
			  if ( (dp & 0x100) == 0 ) {
					...
					if (dp & 0x800) {
							x86r = get_and_lock_68k_reg(map, r, is_dreg, ADDRESS_X86_REGS, 0, 2);
							remove_x86r_from_cache(map, x86r, 0);
							compile_force_byteorder(map, x86r, BO_NORMAL, 0);
							eai->addr_const_off += map->x86_const_offset[x86r];
					} else {
							x86r = get_and_lock_68k_reg(map, r, is_dreg, ADDRESS_X86_REGS, 1, 2);
							remove_x86r_from_cache(map, x86r, 0);
							compile_force_byteorder(map, x86r, BO_NORMAL, 0);
					}
					...
					if (dp & 0x800) {
							if (eai->addr_const_off == 0) {
									if(regd_shift) {
										...
										// lea x86r, [4*x86r+0]
										assemble(0x8D); 
										assemble(0x04 + x86r*8); 
										assemble(0x05 + x86r*8 + lea_mult[regd_shift]);
									}
									...
									/* leal disp8(dispreg,basereg),dispreg */
									assemble(0x8D);
									assemble(0x44 + x86r*8);
									assemble(x86r*8 + tmpr + lea_mult[regd_shift]);
									assemble((uae_u8)eai->addr_const_off);
							} else {
									assemble(0x8D);
							...
							if(regd_shift) {
								// MAYBEDONE
								// _asm int 3
								// lea x86r, [n*x86r + tmpr]
								assemble(0x8D); 
								assemble(0x04 + x86r*8);
								assemble(x86r*8 + tmpr + lea_mult[regd_shift]);
								if(tmpr == r_EBP) {
									// ebp don't have shorter variant.
									assemble_ulong(0);
								}
							} else {
								// add x86r, tmpr
					...
					// 68020/68030/68040
					int r_is_short;
					uae_s32 outer, base_incr;

					r_is_short = ((dp & 0x800) == 0);
					...
					if ((dp & 0x30) == 0x20) {
						base_incr = (uae_s32)(uae_s16)do_get_mem_word((uae_u16 *)p);
						(*pcpp) += 2; p += 2;
					} else if ((dp & 0x30) == 0x30) {
						base_incr = (uae_s32)(uae_s16)do_get_mem_long((uae_u32 *)p);
						(*pcpp) += 4; p += 4;
					} else {
						base_incr = 0;
					}
					...
					if ((dp & 0x3) == 0x2) {
						outer = (uae_s32)(uae_s16)do_get_mem_word((uae_u16 *)p);
						(*pcpp) += 2; p += 2;
					} else if ((dp & 0x3) == 0x3) {
						outer = (uae_s32)(uae_s16)do_get_mem_long((uae_u32 *)p);
						(*pcpp) += 4; p += 4;
					} else {
						outer = 0;
					}

					// For now: simple constant offsets.
					if(reg >= 0) {
						tmpr = get_and_lock_68k_reg(map, reg, 0, ADDRESS_X86_REGS, 1, 1);
						compile_force_byteorder(map, tmpr, BO_NORMAL, 0);
						// eai->addr_const_off = map->x86_const_offset[tmpr];
					} else {
						// TODO
						// _asm int 3
					}
			      ...
			      assemble(0x33);
					...
					if (dp & 0x3) {
						if (dp & 0x4) {
							...
							// base = get_long (base + base_incr);
							if(reg < 0) {
								// MAYBEDONE
								// _asm int 3
								tmpr = get_free_x86_register(map, ALL_X86_REGS);
								lock_reg(map, tmpr, 1);
								// mov tmpr, [base_incr]
								assemble(0x8B);
								assemble(0x05 + tmpr*8);
								assemble_ulong(base_incr + (uae_s32)address_space);
							} else {
								// mov tmpr, [tmpr+base_incr]
								assemble(0x8B);
								assemble(0x80 + tmpr*8 + tmpr);
								assemble_ulong(base_incr + (uae_s32)address_space);
							}
							...
							// return base + r + outer;
							// lea x86r, [tmpr + n*x86r + outer]
							assemble(0x8D);
							assemble(0x84 + x86r*8);
							assemble(x86r*8 + tmpr + lea_mult[regd_shift]);
							assemble_ulong(outer);
						} else {
							// base = get_long (base + r + base_incr);
							if(reg >= 0) {
								// mov x86r, [tmpr + n*x86r + base_incr]
								assemble(0x8B);
								assemble(0x84 + x86r*8);
								assemble(x86r*8 + tmpr + lea_mult[regd_shift]);
								assemble_ulong(base_incr + (uae_s32)address_space);
							} else {
								// mov x86r, [n*x86r + base_incr]
								assemble(0x8B);
								assemble(0x04 + x86r*8);
								assemble(0x05 + x86r*8 + lea_mult[regd_shift]);
								assemble_ulong(base_incr + (uae_s32)address_space);
			        ...
			        assemble(0x0F);
						  assemble(0xC8 + x86r);
							...
							// add x86r, outer
							if(outer != 0) {
								if ((uae_s32)outer >= -128 && (uae_s32)outer <= 127) {
									assemble(0x83);
									assemble(0xC0 + x86r);
									assemble((uae_u8)outer);
								} else {
									assemble(0x81);
									assemble(0xC0 + x86r);
									assemble_ulong(outer);
						...
						// return base + r + (outer + base_incr);
						if(reg >= 0) {
							// lea x86r, [tmpr + n*x86r + outer + base_incr]
							assemble(0x8D);
							assemble(0x84 + x86r*8);
							assemble(x86r*8 + tmpr + lea_mult[regd_shift]);
							assemble_ulong(outer + base_incr);
						} else {
							// lea x86r, [n*x86r + outer + base_incr]
							assemble(0x8D);
							assemble(0x04 + x86r*8);
							assemble(0x05 + x86r*8 + lea_mult[regd_shift]);
							assemble_ulong(outer + base_incr);
...
{
		// Cant put a tag in the middle of an instruction
		// TAG('RMOP');
                        ...
                        int optype)
{
    ...
    if (eais->data_reg == -2) {
        ...
        rmop_finalize(eaid);
        switch(eais->size) {
         case sz_byte: assemble((uae_u8)eais->data_const_off); break;
         case sz_word: assemble_uword((uae_u16)eais->data_const_off); break;
...
static void compile_fetchimm(struct register_mapping *map, struct ea_info *eai, int byteorder)
{
    ...
    switch (eai->size) {
     case sz_byte: assemble(0xC6); assemble(0xC0 + x86r); assemble((uae_u8)eai->data_const_off); break;
     case sz_word: assemble(0x66); assemble(0xC7); assemble(0xC0 + x86r); assemble_uword((uae_u16)eai->data_const_off); break;
    ...
    if (status == CC_TEST_REG) {
        ...
        if (cc_reg == -2) {
            ...
            assemble_long((char*)&regflags);
						...
						// Fix: 0xC7, 0x05 takes two long words (Lauri)
            // assemble_uword(f);
						assemble_ulong(f);
        ...
        if (status == CC_C_FROM_86C && (live_at_end & CC68K_C) != 0)
            ...
            if (live_at_end) {
                if ((status & CC_X_FROM_86C) != 0 && live_at_end == CC68K_X) {
                    ...
                    assemble(0x05); assemble_long( (void *)(4 + (uae_u32)&regflags));
                } else {
                    int tmpr = get_free_x86_register(map, ALL_X86_REGS);
                    /* pushfl; popl tmpr; movl tempr, regflags */
                    assemble(0x9C); assemble(0x58+tmpr);
                             ...
                             int eaino_s, int eaino_d, instrmnem optype)
{
    ...
    if (dstea->mode == Dreg && dstea->data_reg == -1) {
        if (srcea->data_reg == -2) {
            ...
            assemble((uae_u8)srcea->data_const_off);
        } else {
                          ...
                          uae_u8 *realpc, uaecptr current_addr, struct pid_undo *pud)
{
    ...
    wordsizes size = (wordsizes)insn_info[iip].dp->size;
    int shiftcount;
    ...
    compile_prepare_undo(map, (amodes)amode_mode, amode_reg, pud);
    compile_prepareea(map, (amodes)amode_mode, amode_reg, size,
                      &realpc, current_addr,
                                   ...
                                   struct pid_undo *pud)
{
    ...
    compile_prepareea(map, (amodes)insn_info[iip].dp->smode, insn_info[iip].dp->sreg,
                      sz_long, &realpc, current_addr,
                      &eais, 0, EA_LOAD, 1);
    compile_prepareea(map, (amodes)insn_info[iip].dp->dmode, insn_info[iip].dp->dreg,
                      (wordsizes)insn_info[iip].dp->size, &realpc, current_addr,
                      &eaid, 0, EA_LOAD|EA_STORE|EA_MODIFY, 1);

    compile_fetchea(map, &eais, 0, 1);

//MAYBEDONE
		//This seems to be incorrect (Lauri)
    //compile_fetchea(map, &eaid, 1, 1);
		compile_fetchea(map, &eaid, 0, 1);
    ...
    cc_offset = 0; cc_size = (wordsizes)insn_info[iip].dp->size; cc_reg = eaid.data_reg;
...
#if DUMP_CODE
static void dump_block( DWORD mac_addr, const void *buf, size_t count )
{
	char name[_MAX_PATH];
	if(mac_addr) {
		sprintf( name, "drdump_%08x", mac_addr );
	} else {
		strcpy( name, "drdump_anon" );
	}
...
static int m68k_compile_block(struct hash_block *hb)
{
    int movem_extra = 0;
		...
		// if(n_compiled == n_max_comp) _asm int 3
...
#ifdef WIN32
    exits_necessary = (!USER_PROGRAMS_BEHAVE); // == 1
//DOITLATER
		exits_necessary = 0;
#else
...
//DOITLATER: this checksum is not good enough (Lauri)
...
#if BREAK_AT_CODE_BLOCK
						assemble( 0xcc );
#endif
						// if(n_compiled-1 == n_max_comp) assemble( 0xcc );
        }

#if DUMP_CODE
				if(!dump_start_addr) dump_start_addr = insn_info[iip].address;
#endif

#if DISASSEMBLE_68K
				{
				uaecptr addr = insn_info[iip].address;
				uaecptr nextpc;
				m68k_disasm( addr, &nextpc, 1 );
				fflush( stdout );
				fflush( stderr );
				}
#endif
        ...
        if (insn_info[iip].jump_target) {
            if (cesp == CE_STACK_SIZE) {
                ...
                compile_exit_stack[cesp].address = (uae_u32)current_bb->h;
                compile_exit_stack[cesp].jmpoffs = compile_here();
                assemble_ulong(0);
                cesp++;
            }
        ...
        if (insn_info[iip].stop_translation == 1) {
         ...
         case i_RTS:
            ...
            lock_reg(&map, r_EBX, 2);
            {
                char *tmp1, *tmp2, *tmp3;
								...
								// Because of a compiler bug. (Lauri)
								void *jsr_num_ptr = (void *)&jsr_num;
								void *jsr_rets_ptr = (void *)jsr_rets;
								void *jsr_hash_ptr = (void *)jsr_hash;

                /* fetch (A7) */
								// mov ebx, [regs.a7]
                assemble(0x8B); assemble(0x5 + r_EBX*8); assemble_long(regs.regs + 15);
								// mov ebx,[ebx+address_space]
                assemble(0x8B); assemble(0x80 + 9*r_EBX); assemble_long(address_space);
                assemble(0x0F); /* bswapl x86r */
                assemble(0xC8 + r_EBX);
                /* fetch jsr_num */
                assemble(0x8B); assemble(0x5 + r_ECX*8); assemble_long(jsr_num_ptr);
                ...
                compile_org(tmp1);
                assemble(0x89); assemble(0x5 + r_ECX*8); assemble_long(jsr_num_ptr);
         ...
         case i_JMP:
            sync_reg_cache(&map, 1);
            compile_prepareea(&map, (amodes)insn_info[iip].dp->smode,
                              ...
                              eainfo, 0, EA_LOAD, 1);
            {
                ...
                if (eainfo[0].address_reg != -2 || (tmph = get_hash_for_func(eainfo[0].addr_const_off, 1)) == 0) {
                    ...
                    break;
								...
								// Because of a compiler bug. (Lauri)
								void *tmph_execute_ptr = (void *)&(tmph->execute);

                /* check whether the destination has compiled code */
                assemble(0x8B); assemble(r_EBX*8 + 0x05); assemble_long(tmph_execute_ptr);
                assemble(0x09); assemble(0xC0 + 9*r_EBX);
                assemble(0x0F); assemble(0x85); tmp1 = compile_here(); assemble_ulong(0);
                generate_exit(&map, insn_info[iip].address);
         ...
         case i_JSR:
                              ...
                              eainfo, 0, EA_LOAD, 1);
            {
                ...
                assemble(0x8B); assemble(r_ECX*8 + 0x05); assemble_long(jsr_num_ptr);
								// test ecx, MAX_JSRS
                ...
                /* movl $something,disp32(,%ecx,4) */
								...
								// mov [ecx*4+jsr_rets], insn_info[iip+1].address
                assemble(0xC7); assemble(0x04); assemble(0x8d);
                assemble_long(jsr_rets_ptr); assemble_ulong(insn_info[iip+1].address);
								// mov [ecx*4+jsr_hash],(current_bb + 1)->h
                assemble(0xC7); assemble(0x04); assemble(0x8d);
                assemble_long(jsr_hash_ptr); assemble_long((current_bb + 1)->h);
								// inc [jsr_num]
                assemble(0xFF); assemble(0x05); assemble_long(jsr_num_ptr);
                /* Put things on the 68k stack */
								// sub [regs.A7],4
                assemble(0x83); assemble(0x05 + 5*8); assemble_long(regs.regs + 15); assemble(4);
								// mov ecx, [regs+A7]
                assemble(0x8B); assemble(r_ECX*8+ 0x05); assemble_long(regs.regs + 15);
								// mov [ecx+address_space], insn_info[iip+1].address
         ...
         case i_BSR:
                              ...
                              eainfo, 0, EA_LOAD, 1);
            {
                ...
                assemble(0x83); assemble(0x05 + 5*8); assemble_long(regs.regs + 15); assemble(4);
								// mov ecx, [regs.A7]
                assemble(0x8B); assemble(r_ECX*8+ 0x05); assemble_long(regs.regs + 15);
								// mov dword ptr [ecx+address_space],insn_info[iip+1].address
                assemble(0xC7); assemble(0x80 + r_ECX); assemble_long(address_space);
                assemble_ulong_68k(insn_info[iip+1].address);
                /* Off we go */
								// jmp ebx
         ...
         case i_CMP:
         case i_CMPM:
            compile_prepare_undo(&map, (amodes)insn_info[iip].dp->smode, insn_info[iip].dp->sreg, &pub);
            compile_prepare_undo(&map, (amodes)insn_info[iip].dp->dmode, insn_info[iip].dp->dreg, &pub);
            compile_prepareea(&map, (amodes)insn_info[iip].dp->smode,
                              insn_info[iip].dp->sreg,
                              (wordsizes)insn_info[iip].dp->size, &realpc, current_addr,
                              eainfo, 0, EA_LOAD, 1);
            compile_prepareea(&map, (amodes)insn_info[iip].dp->dmode,
         ...
         case i_NEG:
            ...
            compile_prepareea(&map, (amodes)insn_info[iip].dp->smode,
                              insn_info[iip].dp->sreg,
                              (wordsizes)insn_info[iip].dp->size,
                              &realpc, current_addr,
                              eainfo, 0, EA_LOAD|EA_STORE, 1);
         ...
         case i_MVMEL:
            ...
            sync_reg_cache(&map, 0);
            {
                ...
                uae_u16 mask = (uae_u16)eainfo[0].data_const_off;
                ...
                compile_prepareea(&map, (amodes)insn_info[iip].dp->dmode,
                                  insn_info[iip].dp->dreg,
                                  (wordsizes)insn_info[iip].dp->size, &realpc, current_addr + 2,
                ...
                for (i = 0; i < 16; i++) {
                    ...
                    if (mask & 1
                        ...
                        compile_move_reg_from_mem_regoffs(scratch0, scratch2,
                                                          current_offs, (wordsizes)insn_info[iip].dp->size);
                        if (size == 2) {
                ...
                for (i = 0; i < 16; i++) {
                    ...
                    if (mask & 1) {
                        ...
                        compile_move_reg_to_mem_regoffs(scratch2, current_offs,
                                                        scratch0, (wordsizes)insn_info[iip].dp->size);
                    }
                    if (mask & 1)
                        current_offs += size;
                    mask >>= 1;
         ...
         case i_BCHG:
            ...
            handle_bit_insns(&map, eainfo, 0, 1, (instrmnem)insn_info[iip].dp->mnemo);
            break;
    ...
    finish_condjumps(last_iip);
    {
		...
		dump_block( 
			dump_start_addr,
			hb->compile_start, 
			(size_t)(compile_here() - hb->compile_start)
		);
#endif
...
#if DUMP_CODE
		dump_block( dump_start_addr, "FAIL", (size_t)4 );
#endif
...
void compiler_init(void)
{
		...
		address_space = (char *)MEMBaseDiff;
#else
		address_space = (char *)(RAMBaseHost - RAMBaseMac);
#endif
		good_address_map = address_space;
    code_init();
    hash_init();
    jsr_stack_init();
}

void compiler_forget_range(void *start, uint32 size)
{
//TODO
}
...
#ifdef WIN32
/*
 * How come, your code looks quite clear to me.
 */
#endif
