...
prefs_desc common_prefs_items[] = {
	...
	// Toshimitsu Tanaka:
	{"noclipconvert", TYPE_BOOLEAN, false}, // Don't convert clip text (clip_*.cpp)
...
void PrefsInit(void)
{
	...
	// Toshimitsu Tanaka:
	PrefsAddBool("noclipconvert", false);
...
void LoadPrefsFromStream(FILE *f)
{
	...
	while(fgets(line, 255, f)) {
		...
		*p++ = 0;
		...
		// Skip whitespace at the end (easily happens when people copy/paste
		// prefs file contents from WEB pages)
		len = strlen(p);
