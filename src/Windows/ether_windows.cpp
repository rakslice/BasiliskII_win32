...
static int oldest_9x_pending = -1;
...
static unsigned int ether_thread_get_packets_9x(void *arg);
...
void EtherInit(void)
{
  ...
  const char *name;
	...
	if(win_os == VER_PLATFORM_WIN32_WINDOWS) {
		name = PrefsFindString("ether9x");
	} else {
		if(win_os_major == 4) {
			name = PrefsFindString("ethernt");
		} else {
			name = PrefsFindString("ethernt5");
		}
	}
  ...
  // Start packet reception thread
	...
	threads[THREAD_ETHER].h = (HANDLE)_beginthreadex( 0, 0, ether_thread_feed_int, 0, 0, &threads[THREAD_ETHER].tid );
  if (!threads[THREAD_ETHER].h) {
    D(bug("Failed to create ethernet thread\r\n"));
    goto open_error;
  }
	thread_active = true;
	SetThreadPriority( threads[THREAD_ETHER].h, threads[THREAD_ETHER].priority_running );
	SetThreadAffinityMask( threads[THREAD_ETHER].h, threads[THREAD_ETHER].affinity_mask );

	unsigned int dummy;

	if(win_os == VER_PLATFORM_WIN32_WINDOWS) {
		ether_th2 = (HANDLE)_beginthreadex( 0, 0, ether_thread_get_packets_9x, 0, 0, &dummy );
	} else {
...
open_error:
  if (thread_active) {
		TerminateThread(threads[THREAD_ETHER].h,0);
		threads[THREAD_ETHER].h = 0;
		if(int_ack) CloseHandle(int_ack);
		int_ack = 0;
		if(int_sig) CloseHandle(int_sig);
		int_sig = 0;
		if(int_sig2) CloseHandle(int_sig2);
		int_sig2 = 0;
		if(int_send_now) CloseHandle(int_send_now);
		int_send_now = 0;
    thread_active = false;
  }
...
static void cancel_pending_reads(void)
{
	LPPACKET Packet;
	int i, j;

	D(bug("cancel_pending_reads\r\n"));

	if(win_os == VER_PLATFORM_WIN32_NT) return;

	Sleep(100);
	for( i=0; i<PACKET_POOL_COUNT; i++ ) {
		if(packets[i] && !packets[i]->free) {
			Packet = PacketAllocatePacket(fd,256);
			if(Packet) {
				char *b = (char *)Packet->Buffer;
				D(bug("cancelling packet %ld\r\n",i));
				memset( b, 0, Packet->Length );
			  memcpy( b, ether_addr, 6);
			  memcpy( &b[6], ether_addr, 6);
				PacketSendPacket( fd, Packet, TRUE, FALSE );
	...
	if(win_os == VER_PLATFORM_WIN32_WINDOWS) {
		...
		for( j=0; j<PACKET_POOL_COUNT; j++ ) {
			if(packets[i] && !packets[i]->free) {
				D(bug("waiting completion of packet %ld\r\n",i));
  ...
  D(bug("Wait delay\r\n"));
	...
	if(thread_active_3) {
	  D(bug("Ether killing thread\r\n"));
		if(threads[THREAD_ETHER].h) TerminateThread(threads[THREAD_ETHER].h,0);
