...
private:
	...
	static unsigned int input_func(void *arg);
	static unsigned int output_func(void *arg);
...
void SerialInit(void)
{
	...
	debug_serial = PrefsFindInt16("debugserial");
...
unsigned int XSERDPort::input_func(void *arg)
{
	...
	for (;;) {
		...
		if(s->is_file) {
			actual = -1;
			error_code = readErr;
		} else if(!ReadFile(s->fd, buf, length, (uint32*)&actual, 0)) {
			actual = -1;
			if(s->is_serial)
...
unsigned int XSERDPort::output_func(void *arg)
{
	...
	for (;;) {
		...
		int32 actual;
		if(!WriteFile(s->fd, buf, length, (uint32*)&actual, 0)) {
			actual = -1;
			if(s->is_serial)
