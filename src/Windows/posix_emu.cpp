...
void init_posix_emu(void)
{
	...
	for( char letter = 'A'; letter <= 'Z'; letter++ ) {
		...
		switch(GetDriveType(rootdir)) {
			...
			case DRIVE_RAMDISK:
				if(m_use_ntfs_afp) {
					use_streams[ letter - 'A' ] = is_ntfs_volume(rootdir);
				}
				// fall
	...
	GetModuleFileName( (HMODULE)hInst, virtual_root, sizeof(virtual_root) );
	...
	if(my_access(custom_icon_name,0) != 0) {
		...
		if(fd >= 0) {
			...
			if(fd >= 0) {
				...
				my_close(fd);
				set_finder_flags( custom_icon_name, kIsInvisible );
				uint16 flags;
				get_finder_flags( my_computer, flags );
				set_finder_flags( my_computer, kHasCustomIcon | flags );
			} else {
...
{
	DISABLE_ERRORS;
	int result = locking( fd, mode, nbytes );
	if(result < 0) {
		my_errno = errno;
	} else {
		my_errno = 0;
	...
	RESTORE_ERRORS;
	D(bug("locking(%ld,%ld,%ld) = %ld\n", fd, mode, nbytes, result));
	return result;
}
